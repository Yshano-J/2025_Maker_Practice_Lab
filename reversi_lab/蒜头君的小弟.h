...
// 全局变量存储地图大小
static int global_rows = 0;
static int global_cols = 0;

void init(struct Player *player) {
    srand(time(0));
    global_rows = player->row_cnt;
    global_cols = player->col_cnt;
}

// 判断是否为12*12地图
int is_large_map() {
    return global_rows >= 12 && global_cols >= 12;
}
...
// 计算某个位置能翻转的棋子数量
int count_flips(char **board, int rows, int cols, int x, int y, char my, char opp) {
    ...
    for (int d = 0; d < 8; d++) {
        ...
        while (nx >= 0 && nx < rows && ny >= 0 && ny < cols && board[nx][ny] == opp) {
            flips_in_dir++;
            nx += dirs[d][0];
            ny += dirs[d][1];
        }
        
        if (flips_in_dir > 0 && nx >= 0 && nx < rows && ny >= 0 && ny < cols && board[nx][ny] == my) {
...
// 计算棋盘上剩余空格数量
int count_empty(char **board, int rows, int cols) {
    int empty = 0;
    for (int i = 0; i < rows; i++) {
...
// 动态位置权重（含陷阱点和贪心策略）
int dynamic_weight(char **board, int rows, int cols, int x, int y, char my, char opp) {
    // 如果是大地图（12*12），使用简化策略
    if (is_large_map()) {
        // 角落
        if ((x == 0 && y == 0) || (x == 0 && y == cols - 1) ||
    ...
    // 计算当前棋局阶段
    int empty = count_empty(board, rows, cols);
    int total_cells = rows * cols;
    int game_phase;
    
    if (empty > total_cells * 0.6) {
        game_phase = 0; // 开局阶段 (前60%)
    } else if (empty > total_cells * 0.2) {
        game_phase = 1; // 中局阶段 (20%-60%)
    } else {
        game_phase = 2; // 终局阶段 (后20%)
    }
    
    // 计算翻转棋子数量
    int flips = count_flips(board, rows, cols, x, y, my, opp);
...
int evaluate(char **board, int rows, int cols, char my, char opp) {
    ...
    if (is_large_map()) {
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                int w = dynamic_weight(board, rows, cols, i, j, my, opp);
                if (board[i][j] == my) {
                    score += w;
                    my_cnt++;
...
struct Point place(struct Player *player) {
    ...
    // 根据地图大小调整搜索深度
    int search_depth = is_large_map() ? 2 : DEPTH - 1;
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            ...
            do_move(new_board, rows, cols, i, j);
            int score = alphabeta(new_board, rows, cols, search_depth, INT_MIN, INT_MAX, 0, 'O', 'o');
