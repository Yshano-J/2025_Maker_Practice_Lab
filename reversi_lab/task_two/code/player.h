            ...
            for (int j = 0; j < cols; j++) {
                ...
                for (int d = 0; d < 8; d++) {
                    ...
                    while (1) {
                        ni += dirs[d][0];
                        nj += dirs[d][1];
                        if (ni < 0 || ni >= rows || nj < 0 || nj >= cols) break; // 到边界
...
// 进攻与防守兼备的评估函数，主导特征突出，专业级稳定子只在残局极低权重参与
