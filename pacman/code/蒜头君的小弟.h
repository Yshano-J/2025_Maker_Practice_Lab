...
static int nextx[MAXN][MAXM], nexty[MAXN][MAXM];  // 记录到起点的下一步位置
static int ghost_dist[MAXN][MAXM];

// 计算格子到最近破坏者的最短距离（增强破坏者位置有效性判断）
        ...
        for (int j = 0; j < player->col_cnt; ++j) {
            if (dist_map[i][j] >= 0) {
                ...
                if (ok && dist_map[i][j] < best_d) {
                    best_d = dist_map[i][j]; best_x = i; best_y = j;
                }
            }
        }
    }
